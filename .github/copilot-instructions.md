...
2. Last.fm APIからの再生履歴取得（直近指定件数・期間指定・ページネーション対応）
3. Discord Rich Presenceの更新
4. Discord Botチャンネル通知（楽曲変更時）
5. 自動統計レポート送信（日次・週次・月次）
6. 視覚的グラフレポート生成（4種類のグラフを1枚の白背景画像に統合）
7. 実際の聴取データに基づく推移グラフ
8. 環境変数の検証
9. 優雅な終了処理
10. WebサーバーAPI・WebSocket機能
11. レート制限機能
