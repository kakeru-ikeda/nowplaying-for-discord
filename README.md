...
### 7. Spotify API設定（オプション - Discord RPC競合回避用）

Spotifyが再生中の場合、Last.fmのDiscord RPCステータス更新をスキップするために設定：

#### 7-1. Spotify Developer Dashboardでアプリを作成

1. [Spotify Developer Dashboard](https://developer.spotify.com/dashboard)にアクセス
2. 「Create app」をクリック
3. アプリ情報を入力：
   - **App name**: 任意（例：Last.fm Discord Status）
   - **App description**: 任意の説明  
   - **Redirect URI**: `http://127.0.0.1:8888/callback`
   - **Which API/SDKs are you planning to use?**: Web API
4. Client IDとClient Secretをメモ

#### 7-2. Refresh Tokenの取得（重要）
...
1. **認証URLにアクセス**（CLIENT_IDを置き換え）:
   ```
   https://accounts.spotify.com/authorize?response_type=code&client_id=YOUR_CLIENT_ID&scope=user-read-currently-playing%20user-read-playback-state&redirect_uri=http%3A%2F%2Flocalhost%3A8888%2Fcallback&state=abc123
   ```

2. **認証後のURLから認証コードを取得**:
   ```
   http://localhost:8888/callback?code=【この部分をコピー】&state=abc123
   ```

3. **curl でRefresh Tokenを取得**（CLIENT_ID, CLIENT_SECRET, AUTHORIZATIONCodeを置き換え）:
   ```bash
...
4. **レスポンスから`refresh_token`をコピー**

#### 7-3. 環境変数の設定

取得したRefresh Tokenを.envファイルに追加：
...
**必要なスコープ:**
- `user-read-currently-playing`: 現在再生中の楽曲を取得
- `user-read-playback-state`: プレイバック状態を取得

**注意**: Spotify連携を使用しない場合は、`SPOTIFY_ENABLED=false`に設定してください。
...
### Spotify連携機能
- **Spotify再生状態チェック**: Spotifyが再生中の場合、Last.fmのDiscord RPCステータス更新をスキップ
- **競合回避**: SpotifyアプリのDiscordネイティブ統合を優先
- **ユーザー認証**: OAuth 2.0 refresh tokenによるユーザー認証
- **自動判定**: Spotify再生状態を自動チェックしてLast.fmステータスを制御
